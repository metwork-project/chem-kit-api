...
from .data.transformations_from_smiles import (
    ...
    transformations_from_smiles_with_reverse,
)
...
@pytest.mark.parametrize("data", transformations_from_smiles_with_reverse)
def test_transformations_from_smiles_with_reverse_200(client, data):
    body_json = {"smiles": data["smiles"], "reverse": True}
    response = client.post("/transformations_from_smiles", json=body_json)
    assert response.status_code == 200
    assert response.json() == data["results"]
